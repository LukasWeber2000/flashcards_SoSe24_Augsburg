...
void main() {
  group('FlipDeckAlgorithm Tests', () {
    ...
    setUp(() {
      ...
      flashcard = Flashcard(interval: 1.0, ease: 2.5, dueDate: DateTime.now(), question: 'test', answer: 'test', deck: Deck(name: 'deck'));
    });
    ...
    test('processAnswer with "again" updates card correctly', () {
      ...
      // Add more assertions to verify the dueDate has been updated correctly
    });
    ...
    test('processAnswer with "easy" updates card correctly', () {
      ...
      expect(flashcard.interval, 3.25); // Adjusted with defaultEasyBonus
    ...
    test('calculateDueDate returns correct new due date', () {
      ...
      double interval = 10.0; // days
      DateTime expectedDueDate = DateTime(2023, 1, 11);
      DateTime calculatedDueDate = FlipDeckAlgorithm.calculateDueDate(currentDueDate, interval);
